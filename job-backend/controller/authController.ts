...
export const updateProfile = async (req: Request, res: Response) => {
  try {
    const authUser = req.user as { _id?: string; id?: string; role?: string };
    ...
    const {
      ...
      companyName,
      companyLocation,
    } = req.body as Partial<IUser & IAdmin> & { companyName?: string; companyLocation?: string };

    const updates: Partial<IUser & IAdmin> = {};

    if (fullName !== undefined) updates.fullName = fullName;
    if (preferredJobCategory !== undefined) updates.preferredJobCategory = preferredJobCategory;
    if (mobileNumber !== undefined) updates.mobileNumber = mobileNumber;
    if (email !== undefined) updates.email = email.toLowerCase();
    if (companyName !== undefined) (updates as any).companyName = companyName;
    if (companyLocation !== undefined) (updates as any).companyLocation = companyLocation;

    if (req.file) {
      const relativePath = path.join("uploads", "profile", req.file.filename);
      const normalized = relativePath.replace(/\\/g, "/");
      if (isAdmin) {
        (updates as any).logo = normalized;
      } else {
    ...
    let updated;
    if (isAdmin) {
      updated = await Admin.findByIdAndUpdate(userId, updates, {
        new: true,
        runValidators: true,
    ...
    if (!updated) {
      ...
      if (req.file) {
        const filePath = path.join(process.cwd(), (updates as any).profilePicture || (updates as any).logo || "");
